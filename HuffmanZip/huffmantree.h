...
{
public:
    HuffmanTree(LLint weights[]);                           //构造函数，使用权值数组进行构造
    std::map<int, std::string> getCodeTable() { return codeTable; } //获取哈夫曼树对应的编码表
    Node *getRoot() { return root; }                      //获取哈夫曼树根节点的指针

private:
    std::map<int, std::string> codeTable; //哈夫曼编码表
    Node *root;                 //树的跟节点

    Node *mergeTree(Node *tree1, Node *tree2);   //合并两个哈夫曼树
    void makeCodeTable(Node *root, std::string code); //生成哈夫曼编码表
};
